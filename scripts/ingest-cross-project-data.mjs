...
const SESSION_FILES = [
  { file: "combined-real-tasks.json", source: "combined", hoursKey: "actual_hours" },
  { file: "dev-archaeology-real-sessions.json", source: "dev-archaeology", hoursKey: "hours" },
  { file: "epoch-real-sessions.json", source: "epoch", hoursKey: "hours" },
  { file: "github_pipeline-real-sessions.json", source: "github_pipeline", hoursKey: "hours" },
...
for (const { file, source, hoursKey } of SESSION_FILES) {
  ...
  for (const rec of records) {
    ...
    const dedupKey = `${hours}:${category}:${loc}:${files}`;
    ...
    pairs.push({
      estimate: {
        ...
        outputs: {
          ...
          mostLikely,
          pessimistic,
          variance: Math.round(((pessimistic - optimistic) / 6) ** 2 * 10000) / 10000,
          unit: "hours",
        },
