...
CLI_JS="$REPO_DIR/dist/native/epoch-rust-launcher.js"
...
if [[ ! -f "$CLI_JS" ]]; then
  echo "dist/native/epoch-rust-launcher.js is missing; run pnpm run build in $REPO_DIR first." >&2
  exit 1
fi

"$NODE_BIN" "$CLI_JS" telemetry enable --yes --endpoint "$ENDPOINT" >/dev/null
...
/usr/libexec/PlistBuddy -c "Add :ProgramArguments:2 string cd '$REPO_DIR' && '$NODE_BIN' '$CLI_JS' telemetry submit >> '$LOG_FILE' 2>&1 || true" "$PLIST"
