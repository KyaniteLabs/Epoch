...
# Set EPOCH_NUC_HOST (user@host) and EPOCH_LAPTOP_HOST before running.
NUC_HOST="${EPOCH_NUC_HOST:?EPOCH_NUC_HOST must be set (user@host for the receiver)}"
NUC_DATA_DIR="${EPOCH_NUC_DATA_DIR:-/srv/data/epoch}"
LAPTOP_HOST="${EPOCH_LAPTOP_HOST:?EPOCH_LAPTOP_HOST must be set (Tailscale IP or hostname)}"
...
This PR was auto-generated by the weekly consolidation launchd job." \
        --base main \
        --head "$PR_BRANCH" \
    ...
    log "WARN: gh CLI unavailable or unauthenticated; branch pushed without PR"
  fi
