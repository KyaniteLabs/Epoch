...
function cmdEnd(opts) {
  ...
  if (elapsedHours < MIN_HOURS) {
    console.log(`SKIP: ${elapsedHours.toFixed(2)}h is below minimum (${MIN_HOURS}h)`);
  ...
  if (appendFeedback(record)) {
    ...
    console.log(`  Elapsed: ${elapsedHours.toFixed(2)}h (${startedAt.toISOString()} → ${endedAt.toISOString()})`);
  }
...
function cmdGit(opts) {
  ...
  try {
    ...
    const firstTs = execSync(`git log --format=%ci -1 ${fromRef}`, { encoding: "utf-8" }).trim();
    const lastTs = execSync(`git log --format=%ci -1 ${toRef}`, { encoding: "utf-8" }).trim();
    ...
    if (appendFeedback(record)) {
      console.log(`RECORDED: ${record.actualHours}h from git (${fromRef}..${toRef})`);
      console.log(`  ${summary}`);
    }
  } catch (err) {
...
switch (opts.command) {
  ...
  case "git":
    ...
    break;
  default:
    console.log("USAGE: auto-record-actual.mjs <start|end|record|git> [options]");
    console.log("");
    console.log("  start   --task <desc> --estimate-id <id>  Begin tracking");
    console.log("  end     [--estimate-id <id>] [--notes ...] End tracking, record actual");
    console.log("  record  --estimate-id <id> --hours <n>     Quick record N hours");
    console.log("  git     --estimate-id <id> --from HEAD~5   Record from git timestamps");
}
