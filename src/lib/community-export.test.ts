...
function createSampleData(): void {
	...
	writeFileSync(
		...
		JSON.stringify({
			estimateId: "e1",
			actualHours: 12,
			reportedAt: "2026-05-24T20:00:00Z",
		}) +
			"\n" +
			JSON.stringify({
				estimateId: "e2",
	...
	it("records contain no forbidden fields", () => {
		...
		for (const rec of result.dataset.records) {
			for (const key of forbiddenKeys) {
				expect(
					(rec as unknown as Record<string, unknown>)[key],
				).toBeUndefined();
			}
		}
	});
...
describe("validateCommunityExport", () => {
	...
	it("rejects required fields with incorrect types", () => {
		const badPath = join(TEST_DIR, "wrong-required-types.json");
		writeFileSync(
			badPath,
			JSON.stringify({
				_schema: "estimation-record",
		...
		expect(result.errors).toEqual(
			expect.arrayContaining([
				"Record 0: estimated_hours must be a number",
				"Record 0: actual_hours must be a number",
				"Record 0: complexity must be an integer 1-5",
				"Record 0: task_type must be a string",
				"Record 0: timestamp must be a valid ISO date-time",
			]),
		);
	});
});
