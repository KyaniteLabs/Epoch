...
describe("saveConfig", () => {
  ...
  it("overwrites existing config", async () => {
    ...
    saveConfig({
      telemetry: { enabled: false, endpoint: "", lastSubmissionAt: null, lastSubmissionRecordCount: 0, installationId: "v1" },
    });
    saveConfig({
      telemetry: { enabled: true, endpoint: "https://x.com", lastSubmissionAt: null, lastSubmissionRecordCount: 0, installationId: "v2" },
    });
    const config = loadConfig();
    expect(config.telemetry.enabled).toBe(true);
...
describe("isTelemetryEnabled", () => {
  ...
  it("returns false when env var is 0 even if config says enabled", async () => {
    const { saveConfig } = await import("./config.js");
    saveConfig({
      telemetry: { enabled: true, endpoint: "", lastSubmissionAt: null, lastSubmissionRecordCount: 0, installationId: "" },
    });
