...
describe("getCurrentTime", () => {
  ...
  it("returns human-readable format with day of week", () => {
    const result = getCurrentTime("UTC");
    if (!result.ok) return;
    expect(result.data.humanReadable).toMatch(/(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)/);
  });
});

describe("convertTimezone", () => {
  it("converts UTC to America/Los_Angeles", () => {
    const result = convertTimezone("2026-05-01T12:00:00Z", "America/Los_Angeles");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
