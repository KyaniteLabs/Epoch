...
interface ModelProfile {
	...
	tokensPerRound:
		| number
		| { mean: number; meanPrompt: number; meanCompletion: number };
	sampleCount?: number;
}
...
export function notifyToolCall(): void {
	callCounter++;
	if (
		callCounter >= MIN_CALLS_FOR_UPDATE &&
		Date.now() - lastUpdateAt > 86_400_000 &&
		!isUpdating
	) {
		callCounter = 0;
...
export async function updateReferenceDatabase(): Promise<void> {
	...
	if (calibrationRecords.length >= 5) {
		...
		db.toolTaskCorrectionFactors =
			computeToolCorrectionFactors(calibrationRecords);
		db.complexityCorrectionFactors =
			computeComplexityCorrectionFactors(calibrationRecords);
...
export function loadReferenceDb(): ReferenceDatabase | null {
	if (_cachedDb !== undefined && Date.now() - _cachedDbAt < DB_CACHE_TTL)
		return _cachedDb;
	try {
...
export function getTaskTypeCorrectionFactor(taskType: TaskType): number {
	...
	if (db.estimationAccuracy?.correctionFactors?.byTaskType) {
		...
		const factor =
			db.estimationAccuracy.correctionFactors.byTaskType[canaryKey];
...
export function getToolTaskCorrectionFactor(
	tool: string,
	taskType: TaskType,
): number {
	const db = loadReferenceDb();
...
export function getComplexityCorrectionFactor(
	taskType: TaskType,
	complexity: number,
): number | null {
	const db = loadReferenceDb();
...
function mergeBenchmark(
	existing: ToolBenchmark,
	stat: { p50Ms: number; p95Ms: number; meanMs: number; callCount: number },
): ToolBenchmark {
...
function computeCorrectionFactors(
	...
	for (const [type, ratios] of grouped) {
		...
		const median =
			ratios.length % 2 === 0
				...
				: (ratios[mid] ?? 1.8);
		factors[type] =
			Math.round(Math.min(3.0, Math.max(0.1, median)) * 100) / 100;
	}
...
function computeToolCorrectionFactors(
	records: HistoricalRecord[],
...
function computeComplexityCorrectionFactors(
	records: HistoricalRecord[],
