  ...
  app.post("/v1/feedback/record-actual", async (c) => {
    ...
    if (!result.ok) {
      const status = result.reason === "duplicate"
        ? 409
        : result.reason === "below_threshold" || result.reason === "synthetic_id" || result.reason === "unknown_tool" || result.reason === "auto_wallclock_out_of_bounds" || result.reason === "git_derived_out_of_bounds"
