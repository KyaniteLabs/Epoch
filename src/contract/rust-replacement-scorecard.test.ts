...
function writeLedger(path: string, runs: unknown[]): void {
	writeJson(path, {
		version: 1,
		runs,
	});
...
function runScorecard(ledgerPath: string): string {
	return execFileSync(
		"pnpm",
		[
			"exec",
...
describe("formatReplacementScorecard", () => {
	...
	it("accepts durable soak ledgers as single-file scorecard input", () => {
		...
		writeLedger(ledgerPath, [ledgerRun()]);
		const output = runScorecard(ledgerPath);

		expect(output).toContain("verdict:                  REPLACE");
		expect(output).toContain("functional compatibility: 100.00%");
		expect(output).toContain("replacement gates:        25/25 (100.00%)");
		expect(output).toContain('"readyToReplace": true');
	});

	it("blocks ledger scorecards without qualified release performance proof", () => {
		const root = mkdtempSync(join(tmpdir(), "epoch-scorecard-ledger-"));
		const ledgerPath = join(root, "soak-ledger.json");
		...
		expect(output).toContain("first blocker:            qualified-release-performance");
		expect(output).toContain("replacement gates:        24/25 (96.00%)");
		expect(output).toContain('"gate": "qualified-release-performance"');
		expect(output).toContain('"readyToReplace": false');
	});

	it("blocks ledger scorecards when package identity differs from soak identity", () => {
		const root = mkdtempSync(join(tmpdir(), "epoch-scorecard-ledger-"));
		const ledgerPath = join(root, "soak-ledger.json");
		writeLedger(ledgerPath, [
			ledgerRun({
		...
		expect(output).toContain("verdict:                  BLOCKED");
		expect(output).toContain("first blocker:            package-cli-identity");
		expect(output).toContain("replacement gates:        24/25 (96.00%)");
		expect(output).toContain('"gate": "package-cli-identity"');
		expect(output).toContain('"readyToReplace": false');
	});
});
