...
If the estimate writer cannot stamp `inputs.session_id`, pass the session's real time span explicitly: `epoch auto-actuals --session <id> --window-start <ts> --window-end <ts>`. Each `<ts>` may be epoch milliseconds or an ISO timestamp, and both flags must be supplied together with the end no earlier than the start. Epoch uses this window only when the exact session join finds no candidates, and only for pending estimates with no session stamp; estimates stamped to another session are never selected. Window-selected candidates still pass the same sanity and duplicate guards as exact session matches.
