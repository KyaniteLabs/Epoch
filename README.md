...
If the estimate writer cannot stamp `inputs.session_id`, pass the session's real time span explicitly: `epoch auto-actuals --session <id> --window-start <ts> --window-end <ts>`. Each `<ts>` may be epoch milliseconds or an ISO timestamp, and both flags must be supplied together with the end no earlier than the start. Epoch uses this window only when the durable ledger contains no estimate with the exact session ID, including a finalized or expired estimate, and only for pending estimates with no session stamp; estimates stamped to another session are never selected. Window-selected candidates still pass the same sanity and duplicate guards as exact session matches.

**Your git history is another actuals source.** `epoch mine-git --repo <path> --since <date>` walks a local git repository — offline and read-only by construction — and joins pending estimates to closed work (merged PRs and plain merges) via the identity keys planning flows already stamp: `task_label`, `branch`, or `issue_ref`. Two cycle-time windows are derived per unit and reported separately, never blended: first-commit→merge (provenance `git_derived`) and open→merge (provenance `git_derived_review_inclusive`, computed wherever a branch-creation anchor survives locally — plain clones have none, and the window is never fabricated). Pick the recorded window with `--window dev|review-inclusive`; the other rides along in the actual's notes. Mined actuals are sanity-bounded ([0.05h, 720h] — calendar windows legitimately span nights and weekends — and <10x the estimate), never overwrite a real actual, and `feedback_health` segments them in its own `byProvenance` bucket (`gitDerived`) so calendar-window noise cannot silently skew calibration.
