...
- **Fleet-IP hygiene gate now scans `src/` too.** The endpoint-validation test's Tailscale fixture was replaced with the obviously-synthetic in-range `100.100.100.100` (the only allowlisted literal — it exercises the same 100.64.0.0/10 branch), and `scripts/check-no-fleet-ips.mjs` now covers `src/**` as well as docs/scripts/README, so a real fleet address can no longer hide in source.
