...
## [0.5.1] - 2026-09-19 — stdio JSON-RPC protocol hygiene (re-landed 2026-09-25: shipped in npm 0.5.1 from a mirror-side tag, then lost when the 0.5.2 train was cut from canonical main, which had never received these commits; restored for the next release)
...
- **Percentile indexing (nearest-rank/ceil-rank) fixed** in `monte_carlo_schedule`'s P10/P50/P80/P95 and telemetry execution benchmarks: the old floor-rank index was biased one rank high — p95 of an n=20 sample returned the maximum, p50 of n=1000 returned the 501st order statistic. p95 of small samples now sits strictly below the max; the n=1000 median is the 500th order statistic. The live telemetry benchmark path (`getStats`) now uses the same shared ceil-rank helper — an earlier draft of this change fixed only the Monte Carlo path and left the telemetry path on floor-rank.
...
- **UK substitute days** (England & Wales): bank holidays falling on a weekend now substitute the next weekday that is not itself a bank holiday (e.g. Boxing Day Sat 2026-12-26 -> substitute Mon 2026-12-28). The 2025 Early May bank holiday stays on **Monday 5 May** — the VE Day 80th move never took effect (the only VE Day move was 2020; gov.uk confirms 5 May 2025), correcting an earlier draft of this change that applied it.
...
- **US cross-year observed day fixed.** When New Year's Day falls on a Saturday (2022, 2033, …), the preceding-Friday observation is December 31 of the *prior* year — but the observed-day key was only registered in the following year's holiday set, and day-walks/`isBusinessDay` resolve sets by the visited date's own year, so e.g. 2021-12-31 counted as a US business day when it was in fact the observed federal holiday. The prior year's set now carries the key itself.
...
### Changed (hardening series — audit tickets 16-23)
- **Ticket 16 — `unit_suspect` lifecycle.** Actuals whose normalized hours differ from the matched estimate by more than 10x (symmetrically, either direction) are recorded and flagged `unit_suspect`, never silently ingested or dropped; they are excluded from calibration math only when the ratio exceeds 50x. Both transport seams (MCP `record_actual` and the HTTP route) surface the same actionable `flagHint`, pinned by tests.
- **Ticket 17 — stat-keyed ledger read cache.** Ledger reads validate a `(size, mtimeMs, inode)` stat key on every access and re-parse on any mismatch: estimation-call latency no longer grows with ledger history, external appends are picked up immediately, and there is no TTL window to go stale.
- **Ticket 18 — ledger write integrity.** The duplicate-check → append region is serialized by an advisory file lock (token-matched release, dead-owner recovery); failed estimate writes never mint a `feedbackRef` (no phantom tokens); whole-file migrations run under a quiesce lock with atomic tmp+rename rewrites and a tail re-merge that preserves rows appended mid-rewrite.
- **Ticket 19 — telemetry receiver boundary.** `POST /v1/telemetry` validates statistically (ratio consistency, magnitude bounds) and labels integrity at the receive boundary; admission is capped per installation and in total; receipts are appended per receive. **Response semantics:** every current receive path is untrusted (the payload HMAC is keyed by the in-payload `installation_id`, proving integrity but not provenance), so `accepted` is always 0 and every valid record lands in `telemetry-quarantine.jsonl` with a reason label for operator review — the trusted merge store receives nothing until a trusted receive path exists.
- **Ticket 20 — HTTP hardening.** CORS allowlist (no wildcard origins), real body limits that count received bytes chunk-safely and cancel the stream on overflow, and per-IP rate limiting — all enforced before any handler parses a body. The batch record-actual route achieves parity with the single-entry route's error contract.
- **Ticket 21 — self-improve honesty.** Correction-factor updates run off the request path with watermarks (each aggregate reflects only the records it has actually seen), `sampleSize` reports the real matched-pair count, and `auto_wallclock` actuals are provenance-labeled rather than blended with verified ones.
- **Ticket 22 — batch + receiver performance.** `batch_record_actuals` acquires the ledger write lock once per batch (not per entry) and computes in a single pass; the receiver deduplicates via stat-validated in-memory key sets (O(1) per record instead of a full-file re-parse per POST).
- **Ticket 23 — fleet-topology hygiene.** Internal fleet topology (Tailscale addresses, tailnet names, machine inventories) is out of the tracked surface, generated artifacts are untracked, and a CI hygiene gate keeps them out.

### Fixed (independent-review findings on this branch)
- **`POST /v1/feedback/record-actual` maps caller-fixable rejections to 400, not 500.** `unknown_tool` and `auto_wallclock_out_of_bounds` are input/ledger-mismatch problems the caller must fix (the same rejections render as actionable validation errors on the MCP surface); a 500 invited retry loops on unretryable errors and broke clients that branch on status class. `write_failed` remains 500 (server-side lock contention).
- **Storage failures are 500-class at every seam (`errorKind: "storage"`).** Estimate-persistence failure (dispatcher) and all-failed batches whose entries include `write_failed` (dispatcher + HTTP batch route) were previously untagged and mapped to caller-input 422 at the HTTP boundary — permission/disk/lock failures misclassified as input errors, inviting wrong retries. Storage errors keep their crafted-safe messages verbatim (the "NOT recorded / no feedbackRef" contract reaches the caller); thrown internal errors still get the generic-safe 500 replacement.
- **Ledger write locking fails closed and never steals from a live owner.** The advisory lock previously (a) ran the guarded check-then-append region UNLOCKED when the lockfile could not be created (partial-fs/permission environments), and (b) treated any lock older than the staleness window as stealable even when its owner PID was alive — a legitimately slow batch could lose mutual exclusion mid-write. Lock-infrastructure failure is now a `write_failed` result with a permissions-focused hint (`LedgerLockUnavailableError`), and staleness steals require a dead (or absent) owner PID; an old-but-live lock is reported `present, stale: false` in `data_status` with the documented manual recovery path. The migration quiesce lease inherits both semantics through the shared acquisition path.
- **Receiver admission is transactional in ordering and serialized across processes.** The receive path previously persisted the dedup key BEFORE the quarantine record: a failed or crashed quarantine append left a durable key that permanently reported the unstored record as "deduplicated" on every retry (silent data loss), and a failed receipt append permanently undercounted the admission caps. Admission now writes the quarantine record first and the dedup identity second — a mid-admission failure can at worst duplicate a quarantined line on retry, never suppress a record — and the whole caps → dedup → append → receipt sequence runs under a `telemetry-admission.lock` advisory lock (fail-closed 503, retryable) so two receiver processes cannot interleave it.
- **`feedback_health`'s exclusion recount is symmetric with the matcher.** The recount previously evaluated every raw duplicate actual independently while the matcher calibrates only the earliest-reported join winner; a later excluded duplicate could move `seedRecordsFiltered` while a clean earlier winner joined. The recount now iterates the same join winners.
- **Overlay migration writes are counted only when they persist.** `appendOverlayRecord` previously ignored the append result, so a disk/permission failure could still produce a migration report claiming `written: N`; it now throws on a failed append and migrations abort loudly instead of reporting phantom writes.
- **Published output schemas describe the recorded basis.** The `pert_estimate` and `reference_class_estimate` response schemas still described intervals "around adjustedEstimate" (the pre-unification basis this release replaced) and omitted the additive `adjustedEstimate`/`basisNote`/`intervalPopulation` fields; both schemas now name the recorded basis explicitly, and an HTTP-seam test asserts the published `/openapi.json` contract.
- **Live telemetry benchmarks use the shared ceil-rank percentiles.** The floor-rank indexing the release fixed in Monte Carlo survived in `getStats` (execution benchmarks feeding self-improvement), keeping the exact one-rank-high bias; the live path now uses the same helper (p50 pins corrected: n=10 → 50, n=1000 → 500th order statistic).
- **US cross-year observed day.** When New Year's Day falls on a Saturday (2022, 2033, …), the preceding-Friday observation is December 31 of the prior year, but the key only lived in the following year's holiday set — 2021-12-31 counted as a US business day when it was the observed federal holiday. The prior year's set now carries the key.
- **UK 2025 Early May holiday kept on Monday 5 May.** An earlier draft of the calendar-truth fix applied a "VE Day 80th move to 8 May 2025" that never happened (the only VE Day move was 2020; gov.uk confirms 5 May 2025) — golden tests corrected.
- **Japan sandwich-day rule implemented (祝日法第3条第3項).** A non-Sunday weekday between two national holidays is itself a holiday; the 2024-2030 occurrence is 2026-09-22 (between Respect-for-the-Aged Day and the Autumnal Equinox, per the Cabinet Office). Previously the code asserted no such day existed.
- **`unit_suspect` hint states the symmetric detection.** The flag text described only actual-over-estimate overruns; detection is symmetric (`max(actual/estimate, estimate/actual)`), and a 10x overestimate was getting a wrong explanation.
- **Fleet-IP hygiene gate scans every tracked text file.** The endpoint-validation test's Tailscale fixture was replaced with the obviously-synthetic in-range `100.100.100.100` (the only allowlisted literal — it exercises the same CGNAT-range branch), and `scripts/check-no-fleet-ips.mjs` now covers all of `git ls-files` (src, docs, site, skills, .gitea, root configs; lockfiles and the gate itself excluded) and rejects any `*.ts.net` hostname shape — a real fleet address can no longer hide anywhere in the repository.
- **Receiver cap accounting survives a receipt-append failure.** If the quarantine and key appends succeed but the receipt append fails (permissions, ENOSPC edge), the in-process admission caps now still charge what hit disk (accounting runs in the admission lock's finally), and after a restart the total is floored at the record-key file's line count (keys are append-on-admit only). Per-installation counts cannot be repaired post-restart (keys are un-attributable hashes by privacy design) — the total cap backstops them.
- **Business-day tools never silently return weekend-only counts for a wrong country code.** `add_business_days`/`count_business_days` (and `time-math`'s business-day operations) now reject malformed country codes ("USA"-style, any non-2-letter string) with readable validation, and a valid-but-unsupported 2-letter code (e.g. "ES") gets an additive `holidaySupport: "weekends_only"` output field naming the fallback — previously any unknown code silently produced weekend-only counts.
